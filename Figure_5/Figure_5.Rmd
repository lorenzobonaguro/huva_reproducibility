...
# Figure 5 (Supp. Figures S12, S13, S14)
...
DE_HuVa_FG500$PCA_FG500_PBMC + ggtitle("Fig. 5a - PCA 500FG")
```
...
plotEnrichment(hallmarks_V7.2$HALLMARK_INTERFERON_ALPHA_RESPONSE,
               -rank_gene_HuVa$FG500) + labs(title="Fig. S12d - HALLMARK_INTERFERON_ALPHA_RESPONSE") + theme(aspect.ratio = 1/2)
```

#### INTERFERON GAMMA RESPONSE
```{r}
plotEnrichment(hallmarks_V7.2$HALLMARK_INTERFERON_GAMMA_RESPONSE,
               -rank_gene_HuVa$FG500) + labs(title="Fig. S12d - HALLMARK_INTERFERON_GAMMA_RESPONSE") + theme(aspect.ratio = 1/2)
```

#### G2M CHECKPOINT
```{r}
plotEnrichment(hallmarks_V7.2$HALLMARK_G2M_CHECKPOINT,
               -rank_gene_HuVa$FG500) + labs(title="Fig. S12d - HALLMARK_G2M_CHECKPOINT") + theme(aspect.ratio = 1/2)
```

#### INFLAMMATORY RESPONSE
```{r}
plotEnrichment(hallmarks_V7.2$HALLMARK_INFLAMMATORY_RESPONSE,
               -rank_gene_HuVa$FG500) + labs(title="Fig. S12d - HALLMARK_INFLAMMATORY_RESPONSE") + theme(aspect.ratio = 1/2)
```
...
meta.plot_FG500$FG500_PBMC_cellcount$`Classical monocytes (CD14++CD16-)` + 
  ...
  theme_bw() +
  theme(aspect.ratio = 2) +
  ggtitle("Fig S12e - Classical Monocytes") +
  ylab("abs. # class. mono.")
```
...
meta.plot_FG500$FG500_PBMC_cellcount$`Intermediate monocytes (CD14+CD16+)` + 
  ...
  theme_bw() +
  theme(aspect.ratio = 2) +
  ggtitle("Fig S12f - Intermediate Monocytes") +
  ylab("abs. # int. mono.")
```
...
meta.plot_FG500$FG500_PBMC_cellcount$`Non-classical monocytes (CD14++CD16+)` + 
  ...
  theme_bw() +
  theme(aspect.ratio = 2) +
  ggtitle("Fig S12g - Non-classical Monocytes") +
...
meta.plot_FG500$FG500_PBMC_cytokines$IL1b_S.aureus_WB_48h + 
  ...
  theme_bw() +
  theme(aspect.ratio = 2) +
  ggtitle("Fig S12h - IL1b - S.aureus")
```
...
plotPCA(pca_input = batch_dds_vst,
         ...
         ggtitle("Fig. S13b - Batch corrected PCA")
```
...
plotSingleGene(data=batch_vst_anno, 
                 ...
                 shape= NULL) + 
                 theme(aspect.ratio=2) + 
                 ggtitle("Fig. S13c - STAT1 expression") + 
...
#### Plot
```{r}
dotplotGSEA(GSEA_ctrl_unstim$GOup, show = 20) + ggtitle("Fig. S13f - GO enrichment upregulated genes")
```
...
for (i in genes) {
 plot <- plotSingleGene(data=batch_vst_anno, 
                 ...
                 ggtitle("Fig S14a - Comparison of selected genes") +
                 ylab("rlog counts") + xlab("")
 print(plot)
 
 plot_binned <- plot_binned_gene(goi = i, huva_experiment = STAT1_binned)

 print(plot_binned$FG500_PBMC + theme_bw() + theme(aspect.ratio = 2) + ggtitle("Fig S14a - Comparison of selected genes"))
}
```
...
ggplot(gsea_FG500[c(1,2),], aes(x=NES, y=-log10(padj), fill=pathway)) + 
  ...
  geom_hline(yintercept = -log10(0.05), linetype="dashed") +
  geom_vline(xintercept = 0, linetype="dashed") +
  ggtitle("Fig 5e - GSEA DESeq2 DE genes on 500FG huva ranking") +
...
plotEnrichment(DE_genes_list$up_4h_ctrl, -rank_gene_HuVa$FG500) + 
  labs(title="Fig 5f - 500FG - Upregulated genes in STAT1 GOF") + 
...
plotEnrichment(DE_genes_list$dw_4h_ctrl, -rank_gene_HuVa$FG500) + 
  labs(title="Fig 5f - 500FG - Downregulared genes in STAT1 GOF") + 
  theme(aspect.ratio = 1/2)
```
...
ggplot(gsea_ImmVarM[c(1,2),], aes(x=NES, y=-log10(padj), fill=pathway)) + 
  ...
  geom_hline(yintercept = -log10(0.05), linetype="dashed") +
  geom_vline(xintercept = 0, linetype="dashed") +
  ggtitle("Fig S14b - GSEA DESeq2 DE genes on ImmVar HuVa ranking") +
  scale_fill_manual(values = c("#F39B7F", "#01A087"))
```
...
plotEnrichment(DE_genes_list$up_4h_ctrl,
               -rank_gene_HuVa$ImmVarM) + labs(title="FigS14c - ImmVar - Upregulated genes in STAT1 GOF") + theme(aspect.ratio = 1/2)
```

```{r}
plotEnrichment(DE_genes_list$dw_4h_ctrl,
               -rank_gene_HuVa$ImmVarM) + labs(title="FigS14c - ImmVar - Downregulared genes in STAT1 GOF") + theme(aspect.ratio = 1/2)
```
...
# Save environment
```{r}
save.image(paste("./data/",Sys.Date(), "_huva_Figure_5.RData", sep = ""))
```