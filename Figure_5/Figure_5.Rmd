...
ggplot(gsea_ImmVarM[c(1,2),], aes(x=NES, y=-log10(padj), fill=pathway)) + 
  ...
  geom_hline(yintercept = -log10(0.05), linetype="dashed") +
  geom_vline(xintercept = 0, linetype="dashed") +
  ggtitle("Fig 5h - GSEA DESeq2 DE genes on ImmVar HuVa ranking") +
  scale_fill_manual(values = c("#F39B7F", "#01A087"))
```
...
plotEnrichment(DE_genes_list$up_4h_ctrl,
               -rank_gene_HuVa$ImmVarM) + labs(title="Fig 5i - ImmVar - Upregulated genes in STAT1 GOF") + theme(aspect.ratio = 1/2)
```

```{r}
plotEnrichment(DE_genes_list$dw_4h_ctrl,
               -rank_gene_HuVa$ImmVarM) + labs(title="Fig 5i - ImmVar - Downregulared genes in STAT1 GOF") + theme(aspect.ratio = 1/2)
```
