...
#GMT
  ...
  if(!is.null(egmt)){
    ...
    container[["HALLMARK_table"]] <- egmt@result
    container[["HALLMARK_plot"]] <- hallmark_plot
...
#KEGG
  ...
  if(!is.null(kegg_enrich) && kegg_enrich@result %>%nrow()>0){
    ...
    container[["KEGG_table"]] <- kegg_enrich@result
    container[["KEGG_plot"]] <- kegg_plot
  }
...
#Reactome
  ...
  if(!is.null(reactome_enrich)){
    ...
    container[["Reactome_table"]] <- reactome_enrich@result
    container[["Reactome_plot"]] <- reactome_plot
  ...
  if(!is.null(DO_enrich)){
    ...
    container[["DO_table"]] <- DO_enrich@result
    container[["DO_plot"]] <- DO_plot
    ...
    #@shobhit add a column for gene symbol, shift the geneid in the go result to symbol and find the corresponding ENTREZID for geneId
  {
    ...
    container[["GO_table"]] <- GO_enrich@result
    container[["GO_plot"]] <- enrichGO_plot
...
#TF prediction
 ...
 #    
 #    container[["TF_table"]] <- TFtable
 #    container[["TF_plot"]] <- plotTheThing()
 #  }
